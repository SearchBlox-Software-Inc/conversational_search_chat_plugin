...
export const getAutoSuggest = (query) => {
  ...
  const filterName = useSearchStore.getState().filterName;
  ...
  if(defaults.smartSuggest.enable !== "" && defaults.smartSuggest.enable && window.smartSuggest.SmartSuggest.length === 0 && (defaults.smartSuggest.cname || filterName.smartSuggest)){
      const { domain, isSBDomain, cname, limit, language } = defaults.smartSuggest;
      const smartSuggestName = filterName.smartSuggest || cname;
      const endpointPath = isSBDomain ? `/rest/v2/api/smartsuggest` : `/SmartSuggest`;

    return axios.get(domain + endpointPath + "?q=" + query + "&cname=" + smartSuggestName + "&limit=" + limit + "&lang=" + language)
    .then((response)=>{
      return response;
    })
    .catch((error)=>{
...
export const getTrendingData = () => {
  const filterName = useSearchStore.getState().filterName;
  const cname = filterName.trendingSearch || defaults.trendingSearch.cname;
...
export const getSmartFAQS = (query, limit) => {
  ...
  const collectionParameter = defaultCollections?.length ? `&col=${defaultCollections}` : filterName?.colId && filterName.colId !== "all" ? `&col=${filterName.colId}` : '';
