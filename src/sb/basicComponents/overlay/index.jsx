...
function Overlay({ open, children, className }) {
   ...
   return (
      <dialog className={`${styles.sbModal} ${className}`} ref={modalRef}>
         { children }
      </dialog>
   );
}
...
function OverlayTitle({ onClose, children, className }) {
...
function OverlayContent({ children, className }) {
   return (
      <div className={`${styles.modalBody} ${className}`} tabIndex="0">
         { children }
      </div>
   );
}
